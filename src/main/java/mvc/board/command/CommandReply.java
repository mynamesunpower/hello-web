...
public class CommandReply implements Command {
	...
	public CommandReply (String nextPage) {
	...
	public String execute(HttpServletRequest request, HttpServletResponse response) throws CommandException {
		try {
			...
			BoardVO replyArticle = new BoardVO();
			replyArticle.setWriterName(request.getParameter("writerName"));
			replyArticle.setContent(request.getParameter("title"));
			replyArticle.setTitle(request.getParameter("content"));
			replyArticle.setPassword(request.getParameter("password"));

			this.reply(parentId, replyArticle);

			return nextPage;
		} catch (BoardException e) {
			throw new CommandException("CommandReplay.java 답글 에러:" + e.getMessage());
		}
	}
}
