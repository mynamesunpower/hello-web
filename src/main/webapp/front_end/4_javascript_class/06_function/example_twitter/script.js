...
function renderOrginalTweet(){
	// data.js에 있는 DATA 값을 출력
    for (let i = 0; i < DATA.length; i++){
        let li = createTweetEl(DATA[i]);
...
//*2
randomTweetBtn.onclick = renderRandomTweet;

function renderRandomTweet(){
    if(randomTweetBtn.className === 'pushRandomTweet'){
        let randomData = generateNewTweet();		// data.js 에 있는 함수
        let randomLi = createTweetEl(randomData);
        target.prepend(randomLi);
...
wBtn.onclick = renderNewTweet;

// (함수) 새로운 트윗을 화면에 출력
function renderNewTweet(){
    let newData = {};
    if (userName.value !== '' || msg.value !== ''){
        newData.user = userName.value;
        newData.message = msg.value;
        newData.created_at = new Date().format();
        let newLi = createTweetEl(newData);
        target.prepend(newLi);
